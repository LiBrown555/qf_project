...
url = iot_device_config['device_status_post']['url']
...
class TaskExec:
    def __init__(self):
        ...
        self.log_init.log_init(log_path, "task_exec")
    ...
    def _get_device_config(self):
        """
            根据设备类别获取对应设备配置
            返回信息格式为
            {'24': {'ip': '192.168.10.201', 'port': 1350, 'username': None, 'password': None, 'parameter': None}, 
            '25': {'ip': None, 'port': None, 'username': None, 'password': None, 'parameter': {'group_id': 1}}, 
            '26': {'ip': None, 'port': None, 'username': None, 'password': None, 'parameter': {'group_id': 2}}, }
        """
    ...
    def _task_exec(self, task_list: list):
        ...
        for task in task_list:
            ...
            if flag:
                if result:
                    task_exec_db.update_task_progress(int(task["id"]), 0)
                    time.sleep(5)#睡眠5s，留出程序推送端侧时间，防止数据推送时。端侧未保存任务进度导致失败
                    for action in result:
                        host_type, result = task_exec_db.get_device_host_type(int(action["device_id"]))
                        if host_type != -1:
                            if host_type == 13: #执行温振传感器主机任务
                                host_config = self.device_config[str(result)]
                                task_mes = {"ip": host_config["ip"], "port": host_config["port"], "group_id": self.device_config[str(int(action["device_id"]))]["parameter"]["group_id"]}
                                # print(task)
                                # print(f"待执行任务:{action}")
                                # print(f"任务参数: {task_mes}")
                                self._wenzhen_sensor_host_task(task, action, task_mes)
                            else:
                                print(f"非温振传感器主机,跳过执行: {action}")
                        else:
                            self.logger.error(f"获取设备主机类型失败,错误信息: {result}")
                    task_exec_db.update_task_progress(int(task["id"]), 1)
                else:
                    self.logger.info(f"未获得到任务对应的动作,更新任务进度为已完成")
                    task_exec_db.update_task_progress(int(task["id"]), 1)
            else:
                task_exec_db.update_task_progress(int(task["id"]), 2)
                self.logger.error(f"获取任务动作失败,错误信息: {result}")

    def _wenzhen_sensor_host_task(self, task: dict, action: dict, task_config: dict)->dict:
        """
        执行温振传感器主机任务
        task: 任务计划
        action: 任务对应的动作
        ...
        try:
            recv_data = {}
            data = wenzhen_reader.get_gateway_data(task_config["ip"], task_config["port"], group_ids=[task_config["group_id"]])
            sensors = data.get("sensors", {})
            # 交互模式下，仍然使用原来的单传感器展现形式
            if not sensors:
                recv_data ={
                    "sensor_id": task_config["group_id"], 
                    "error": "指定的传感器不存在或无数据",
                    "recv_status": False
                    }
            else:
            ...
            recv_data =  {"sensor_id": task_config["group_id"], 
                    "error": "连接超时",
                    "recv_status": False
                    }
        except OSError as e:
            ...
            if recv_data["recv_status"]:#传感器信息回传正常
                flag , item_definition_list = task_exec_db.get_item_definition(action["device_id"])
                if flag:#获取设备定义成功
                    if item_definition_list:#设备定义列表不为空
                        item_values = {}
                        for item_definition in item_definition_list:
                           item_values[item_definition["item_code"]] = recv_data[item_definition["item_code"]]
                        save_mes["item_values"] = item_values
                        save_mes["post_status"] = 1
            else:
                save_mes["error_info"] = recv_data["error"]
                save_mes["post_status"] = 2
        self.logger.info(f"保存任务结果: {save_mes}")
        flag, result = task_exec_db.save_task_result(save_mes)
        if not flag:
            self.logger.error(f"保存任务结果失败,错误信息: {result}")
        else:
            # self.task_mes_post(task,action,save_mes)
            self._save_task_result(task,action,save_mes)
    ...
    def task_mes_post(self, task: dict, action: dict,save_mes: dict):
        """
        推送任务结果到端侧平台
        task: 任务计划
        action: 任务对应的动作
        """
        flag, device_code = task_exec_db.get_device_code_by_int(save_mes["device_id"])
        if flag:
        ...
        post_data = {
            ...
            "error_info": save_mes["error_info"],
            "data": {
                "task_progress_uuid": task["progress_uuid"],
                "task_action_id": action["id"],
                "task_action_param": action["action_params"],
                "file_parameter": save_mes["item_values"]
            },
        }
        post_data = json.dumps(post_data, ensure_ascii=False).encode('utf-8')
        headers = {"Content-Type": "application/json; charset=utf-8"}
        if save_mes["file_path"]:
            with open(save_mes["file_path"], "rb") as file:
                response = requests.post(url, data=post_data, files={"file": file}, headers=headers)
                if response.status_code == 200:
                    self.logger.info(f"推送任务结果成功: {response.json()}")
                else:
                    self.logger.error(f"推送任务结果失败: {response.json()}")
        else:
