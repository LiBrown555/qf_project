...
class InspectionItems(Base):
    __tablename__ = 'algorithm_inspection_items'
    id = Column(Integer, primary_key=True, autoincrement=True, comment="检测项ID(唯一标识)")
    inspection_item_name = Column(String(255), nullable=False, comment="检测项名称")
    inspection_item_type = Column(Integer, nullable=False, comment="检测项数据类型(0:string,1:int,2:float,3:bool)")
    algorithm_id = Column(Integer, nullable=False, comment="所属算法ID")
    create_time = Column(DateTime, nullable=False, comment="创建时间")
    def __repr__(self):
        return f"InspectionItems(id={self.id}, inspection_item_name={self.inspection_item_name}, inspection_item_type={self.inspection_item_type}, algorithm_id={self.algorithm_id}, create_time={self.create_time})"
    
    def to_dict(self):
        return {
            "id": self.id,
            "inspection_item_name": self.inspection_item_name,
            "inspection_item_type": self.inspection_item_type,
